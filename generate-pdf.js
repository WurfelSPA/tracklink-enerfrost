 ...
 *   - CONFIG.speedThreshold = 120 → filtra filas con velocidad < umbral
 *     (la API de TrackGTS no acepta umbral como parámetro; siempre devuelve
...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIG — lo único que cambia entre clientes (KADEL / ENERFROST / futuros)
// ─────────────────────────────────────────────────────────────────────────────
const CONFIG = {
  siteName:       'ENERFROST',
...
// TODO: completar si ENERFROST tiene nómina de conductores con tildes/formato
// especial, igual que NAME_ACCENT_FIX en tracklink-santamarta.
...
async function main() {
  ...
  console.log(`[pdf] Generando reporte ${CONFIG.siteName}: ${startDate} → ${endDate}`);
  ...
  console.log(`[pdf] Filas en el período (>= ${CONFIG.speedThreshold} km/h): ${rows.length}`);
...
function generateHTML(s) {
  ...
  const hourChartSvg   = verticalBarChart(hourChartItems, { width: 1168, height: 380, peakBadge: `Máximo: ${s.peakHour.count} excesos`, xAxisLabel: 'Franja Horaria', yAxisLabel: 'Excesos', idPrefix: 'chartHour' });
...
<div class="page cover">
  <div class="cv-left">
    <svg width="360" height="230" viewBox="0 0 360 230" fill="none">
      ...
      <circle cx="105" cy="188" r="17" fill="${CONFIG.colorDarker}"/><circle cx="105" cy="188" r="8" fill="#fff" opacity=".55"/>
      <circle cx="240" cy="188" r="17" fill="${CONFIG.colorDarker}"/><circle cx="240" cy="188" r="8" fill="#fff" opacity=".55"/>
